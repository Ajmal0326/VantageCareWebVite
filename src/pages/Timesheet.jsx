...
/** Normalize a time string into HH:mm (accepts 6pm, 6:30PM, 18, 18:0, 18.00, 18h00, etc.) */
function normalizeHM(raw) {
  const s = safeStr(raw).toLowerCase().replace(/\s+/g, "");
  if (!s) return "";

  // 6pm / 6:30pm / 12am / 12:45am
    ...
    const fetchMyWeek = async () => {
      ...
      try {
        ...
        usersSnap.forEach((docSnap) => {
          ...
          if (data.email === user.email) myData = data;
        });

        const all = Array.isArray(myData?.shifts) ? myData.shifts : [];
        const norm = all.map(normalizeShift);
          ...
          <table className="min-w-full text-left">
            ...
            <tbody>
              {myWeekShifts.length === 0 ? (
                <tr>
                  <td className="py-3 px-3 sm:px-4 text-sm text-gray-500" colSpan={4}>No assigned shifts this week.</td>
                </tr>
              ) : (
                myWeekShifts.map((s, idx) => {
                  const { start, end, overnight } = parseShiftToRange(s);
                  ...
                  return (
                    <tr key={`${s.shiftDate}-${s.shiftStartTime || idx}`} className={idx % 2 === 0 ? "bg-blue-50/40" : "bg-white"}>
                      <td className="py-2 px-3 sm:px-4">{s.shiftDate}</td>
                      <td className="py-2 px-3 sm:px-4">
                        {(safeStr(s.shiftRole) || "Shift")} • {startLabel}{endLabel ? ` – ${endLabel}` : ""}
                      </td>
                      <td className="py-2 px-3 sm:px-4">{shiftHours(s).toFixed(2)} hr</td>
                      <td className="py-2 px-3 sm:px-4">
                        <span
